...
- 누군가 문자열을 StringCalculator에 입력한다.
    - (1) StringCalculator은 DelimiterPolicy로 하여금 커스텀 구분자 여부를 확인후, delimiter와 calculationString을 추출할 것을 요청함
    - DelimiterParser가 delimiter과 calculationString을 추출함으로써 응답함.
    - (2) StringCalculator은 Tokenizer 에게 delimiter와 calculationString에서 정수를 분리할 것을 요청함.
    - Tokenizer는 정수를 분리하여 Operand에 담음으로써 응답함
    - (3) StringCalculator은 Operands에게 분리한 정수를 다 더할 것을 요청함
    - Operands는 값을 더함으로써 응답함

### 책임 : 하는것과 아는것

- StringCalculator(문자열 계산기)
    - 하는 것 : DelimiterPolicy와 Tokenizer와 Operands에 각각 요청함
...
- Tokenizer(정수 추출기)
    - 하는것 : calculationString에서 delimiter을 사용하여 정수를 분리하여 Operands 생성 + validation
...
- ‘//’ 와 ‘\n’의 위치 파악하기
    - `//` 와`\n` 가 문자열 내에 둘 다 존재하는 경우
        - 사이의 커스텀 구문자 확인
            - 커스텀 구분자의 길이가 1이 아닌경우. 구문자가 없거나 길이가 2 이상
...
- delimiter와 calculationString 전달 받기.
- calculationString 을 순회하며 숫자부분으로 이루어져 있는지 확인하고 → int 로 바꿔주기
...
Operands : Tokenizer에서 분리된 정수 array를 받고 합을 연산하는 부분
...
- 기존 custom 문자도 추가할 수 있는가? → 그렇다

```jsx
//:\n1:2.3 -> 6 
```

- 숫자 사이사이 여러개의 구분자 사용 가능? → 그렇다
  - "" 자체도 0으로 인식하는 것으로 봐서 가능하다 판단 
```jsx
1::2,,:3
```
...
- 지정되지 않은 custom 문자 사용될 경우

```jsx
1+2+3 -> error
```

- 문자열에 음수 or 0이 사용된 경우
